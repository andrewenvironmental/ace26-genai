...
- Keep public docs ranked: Azure web app first, local/app-service fallback second, offline notebook last.
...
- PowerPoint files (`*.ppt`, `*.pptx`): keep local/private if needed, but do not track in the public app branch because presentation files can contain author metadata and attendee/instructor PII.
...
- Notebook fallback runbooks: keep local-only with the notebook or replace with a short public note if the fallback becomes part of participant instructions.
- `notebooks/README.md`: delete if the `notebooks/` directory is removed.
- PR description drafts: keep out of the public docs because they are historical process text, not durable workshop documentation.
